...
[真题练习](https://csgraduates.com/study_methods/tags/408quiz/#%e4%ba%8c%e5%8f%89%e6%a0%91%e9%81%8d%e5%8e%86)
...
- 每一层都被完全填满。
- 第 n 层的结点数量为 $2^n$（这里假设根结点是第 0 层）。
- [高度](https://csgraduates.com/data_structure/tree/tree/#%e9%ab%98%e5%ba%a6) 为 h 的满二叉树节点数目为 $$n = 2^h - 1$$
- 结点数量为 n 的满二叉树的高度为 $$h = \log_2(n+1)$$
...
- [高度](https://csgraduates.com/data_structure/tree/tree/#%e9%ab%98%e5%ba%a6) 为 h 的完全二叉树的结点数量范围为 $[2^{h-1}, 2^h - 1]$。
- 结点数量为 n 的完全二叉树的高度为 $$h = \log_2(n+1)$$
...
**平衡二叉树**（Balanced Binary Tree），也叫做 **AVL 树**。它的特点是任意节点的左右子树高度差（**平衡因子**）不超过 1，这确保了树的高度始终保持在 O(log2​n) 的水平，使得查找、插入和删除操作的时间复杂度都保持在 O(log2​n) 。
